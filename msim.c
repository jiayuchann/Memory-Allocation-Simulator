...
typedef struct block{
    int payloadIndex; //index of payload
    int size;
    int free;
    struct block *next;
    struct block *prev;
...
// explicit list storing free blocks, LIFO policy
...
void* heapArray[1000]; // stores payloads
...
void split(Block* block, size_t size, ExpList* freeBlock){
    ...
    if (nextFooter > heapSize/sizeof(WORD)){
        nextFooter = heapSize/sizeof(WORD);
    }

	unsigned int nextSize = (nextFooter - nextHeader) * sizeof(WORD);

	if (nextSize < sizeof(WORD) * 2){
        return;
    }
    ...
    if (nextBlock->size % sizeof(WORD) > 0){
        sz++;
    }
        ...
        if (block->free){
            if (block->size >= size){
                if (block2 == 0){
                    ...
                    if ((block->size-size) < (block2->size-size)){
                            block2 = block;
                    }
                }
            }
...
ExpList* bestFitExp(int size){
    ...
    while (freeBlock != 0){
        if (freeBlock->block->size >= size){
            if (freeBlock2 == 0){
                freeBlock2 = freeBlock;
            }else{
                if ((freeBlock->block->size-size) < (freeBlock2->block->size-size)){
                    freeBlock2 = freeBlock;
                }
            }
        }
...
void* myalloc(int size){
    ...
    if (freeList == 'I'){
        if (fit == 'F'){
            block = firstFitImp(size);
        }
        else if (fit == 'B'){
            block = bestFitImp(size);
        }
    }
...
int main(int argc, char *argv[]){
    ...
    while (fgets(str, 100, inputfile)){
        ...
        while (1){
            ...
            if (temp == 2){
                secondParam = atoi(token);
            }
            else if (temp == 3){
                thirdParam = atoi(token);
            }
            else if (temp == 4){
                fourthParam = atoi(token);
            }
            temp++;
        }
        ...
        switch (*action){
            case 'a':
                pointer = myalloc(secondParam);
				heapArray[thirdParam] = pointer;
                break;
            
            case 'r':
                pointer = heapArray[thirdParam];
                if (secondParam > 0){
                    pointer = myrealloc(pointer, secondParam);
                    heapArray[fourthParam] = pointer;
                }
                else{
                    myfree(pointer);
                }
